...
with open("output/crystal.xyz","w+") as xyzFile:
...
with open("output/POSCAR","w+") as vaspFile:
    vaspFile.write(comment)
    vaspFile.write("\n1.0\n")
    for m,d in zip(multiplyers,directions):
        for field in d: 
            vaspFile.write("  %.10f"%(m*field))
        vaspFile.write("\n")
    for atomName in atomNames:
        vaspFile.write("%s "%atomName)
    vaspFile.write("\n")    
    for atomNumber in cellAtomsCopy:
